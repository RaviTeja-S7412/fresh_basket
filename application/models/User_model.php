...
class User_model extends CI_Model
{
	...
	public function register($data)
	{
		...
		$check = $this->db->where(array('user_mobile' => $this->input->post('user_mobile'), "user_status" => 0))->get('shreeja_users');
		if ($check->num_rows() > 0) {
		...
		$query = $this->db->insert("shreeja_users", $content);
		if ($query) {
			$data = array("status" => true, "message" => "registered successfully", "steps_completed" => 4);
		} else {
			$data = array("status" => false, "message" => "Something went wrong , please try again");
		}
